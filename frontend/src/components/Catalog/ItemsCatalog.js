...
const Item_catalog = ({item, i, addInBucket = f => f}) =>
    ...
    <div key={i} className='item_catalog card_catalog'>
        <Link to={`/item?id=${item.id}`}><img
            src={item.images !== undefined && item.images[0] !== undefined ? item.images[0].image : notFound}
            className='item_img'/></Link>
...
class Catalog extends Component {
    ...
    render() {
        ...
        return <div className='catalog'>
            {Array.isArray(catalog) ? catalog.map((item, i) => <Item_catalog item={item} key={i}
                                                                             addInBucket={this.addInBucket}/>) :
